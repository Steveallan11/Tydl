...
interface CleanerOnboardingModalProps {
  onClose: () => void;
  onSuccess: () => void;
}

export function CleanerOnboardingModal({ onClose, onSuccess }: CleanerOnboardingModalProps) {
  const { onboardCleaner } = useAdmin();
      ...
      <div className="fixed inset-0 flex items-center justify-center z-50 p-4">
        <Card className="w-full max-w-2xl max-h-[90vh] overflow-y-auto">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-2xl font-bold text-slate-900">Onboard New Cleaner</h2>
            <button
              onClick={onClose}
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {/* Compensation */}
            <div>
              <h3 className="text-lg font-semibold text-slate-900 mb-4">Compensation</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-2">
                    Hourly Rate (£)
