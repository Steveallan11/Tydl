...
export function CleanerLogin() {
  const navigate = useNavigate();
  const { login, error: authError, clearError } = useCleanerAuth();
    ...
    <main className="bg-gradient-to-br from-brand-600 via-brand-500 to-blue-600 min-h-screen flex items-center justify-center px-6 py-12">
      <Card className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-slate-900 mb-2">Cleaner Login</h1>
          <p className="text-slate-600">Access your jobs and profile</p>
        </div>
        ...
        <div className="mt-6 pt-6 border-t border-slate-200 text-center">
          <p className="text-slate-600">
            ...
            <Link to="/cleaner/signup" className="text-brand-600 font-semibold hover:text-brand-700">
              Sign up
            </Link>
          </p>
        </div>
